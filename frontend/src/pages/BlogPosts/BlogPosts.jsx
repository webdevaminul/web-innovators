...
const BlogPosts = () => {
    ...
    const categories = ['All', 'Technology', 'Health', 'Education', 'Entertainment']; // Update as per your categories
        ...
        <div className="bg-white text-gray-800 min-h-screen overflow-hidden">
            <div className="container mx-auto grid grid-cols-1 md:grid-cols-6 lg:grid-cols-5 gap-4">
                ...
                <div className="col-span-full md:col-span-4 lg:col-span-4 p-4">
                    {selectedPost ? (
                        <div className="bg-gray-100 mb-4 p-12 w-full rounded-lg shadow-md">
                            <button className="text-violet-500 mb-2" onClick={handleBackClick}>
                                &lt; Back to Posts
                            </button>
                            <div
                        ...
                        currentPosts.map((post) => (
                            <div key={post.id} className="bg-gray-100 p-8 mb-4 rounded-lg shadow-md">
                                <div
                                    className="bg-no-repeat bg-cover h-64 rounded-lg mb-2"
                                    style={{
                                        backgroundImage: `url(${post.image})`,
                ...
                {/* Right Sidebar (Visible on Medium and Large Devices) */}
                <div className="hidden lg:flex lg:col-span-1 relative z-10">
                    <div className="bg-gray-100 p-4 rounded-lg shadow-md">
                        <h2 className="text-xl font-semibold mb-2">Latest Posts</h2>
                        {latestPosts.map((post) => (
                            <div key={post.id} className="flex mb-2">
                                <div
                                    className="w-16 h-16 bg-no-repeat bg-cover rounded-lg mr-2"
                                    style={{
                                        backgroundImage: `url(${post.image})`,
                                        backgroundPosition: 'center',
                                        backgroundSize: 'cover',
                                ...
                                ></div>
                                <div>
                                    <h3 className="text-lg font-semibold">{post.contentTitle}</h3>
            ...
            <div className="flex justify-center mt-4">
                {Array.from({ length: totalPages }, (_, index) => (
                    <button
                        key={index + 1}
                        className={`mx-1 px-4 py-2 border rounded ${currentPage === index + 1 ? 'bg-violet-400 text-white' : 'bg-white text-violet-500'}`}
                        onClick={() => handlePageChange(index + 1)}
                    >
                        {index + 1}
                    </button>
