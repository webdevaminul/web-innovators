  ...
  const renderCategory = (category) => {
    ...
    return (
      <div className="category-section mt-15" key={category}>
        <h2>{category} ({categoryCourses.length} Courses)</h2>
  ...
  return (
    <div className="container">
      ...
      <div className="banner-section relative h-screen w-full flex flex-col md:flex-row items-center justify-between bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400">
        <div className="banner-overlay absolute inset-0 bg-black opacity-50"></div> {/* Semi-transparent overlay for readability */}
        
        {/* Banner content */}
        <div className="banner-content relative z-10 mt-20 text-left text-white p-10 w-full md:w-1/2 text-center md:text-left">
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight leading-tight">
            Master New Skills
          </h1>
          <p className="text-xl mb-4 md:text-2xl mt-4 mb-4 font-light">
          ...
          <div className="search-bar mt-8 flex flex-col md:flex-row items-center w-full">
            <input
              type="text"
              className="p-3 w-full md:w-96 rounded-md border-none outline-none text-black"
              placeholder="Search by category..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <button className="bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-3 mt-4 md:mt-0 md:ml-2 rounded-md w-full md:w-auto">
              Search
            </button>
          </div>
        </div>
      ...
      {/* Courses Section */}
      <h1 className="page-header mt-10 text-3xl font-bold text-center md:text-left">Skill Courses</h1>
